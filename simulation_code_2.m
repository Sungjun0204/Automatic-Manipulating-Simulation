% S자 모양의 좁은 통로에서 로봇 팔과 점의 임피던스 제어 시뮬레이션 (목적지까지 이동)
...
m_device = 0.5; % 장치의 질량 [kg]
I_device_cm = (1/12)*m_device*(0.2^2); % 장치의 자체 관성 모멘트 [kg*m^2] (예: 길이 0.2m인 막대 기준)
L_total = L1 + L2; % 장치의 회전축까지의 거리 [m]
...
% 자석 효과를 위한 상수 설정
magnet_force_constant = 1.5; % 자석 인력 상수
...
% 초기 관절 각도 및 속도
start_point = [-1.5; -1.5]; % S자 통로의 시작 지점
...
% 로봇 팔의 목표 위치
end_point = [1.5; 0.5]; % S자 통로의 끝 지점

% S자 통로의 좌표 정의 (폭이 0.3인 좁은 통로)
% S자 곡선을 정의하기 위해 다수의 점을 사용
num_curve_points = 100;
x_curve = linspace(-1.5, 1.5, num_curve_points);
y_curve = sin(pi * x_curve / 3); % S자 곡선 정의 (주기와 진폭 조절 가능)
maze_x_center = x_curve;
maze_y_center = y_curve - 0.5;
...
% S자 통로 그리기
plot_maze(maze_x_center, maze_y_center, maze_width);
...
for i = 1:length(time)
    ...
    % 점이 S자 통로 내부에 있는지 확인
    [inside_maze, p_corrected] = check_maze(p, maze_x_center, maze_y_center, maze_width);
    if inside_maze
        p = p;
    else
    ...
    % 동역학 방정식 계산
   [M, C, G] = dynamics_matrices(q, qd, m1, m2, L1, L2, I1, I2, m_device, I_device_cm, L_total);
    ...
    if mod(i, 20) == 0
        cla;
        % S자 통로 그리기
        plot_maze(maze_x_center, maze_y_center, maze_width); hold on;
        % 로봇 그리기
        draw_robot(q, L1, L2);
        % 점 그리기
...
xlabel('X Position [m]');
ylabel('Y Position [m]');
legend('Maze', 'End Effector Trajectory', 'Point Trajectory', 'Goal Position');
title('로봇 팔과 점의 궤적');
grid on;
axis equal;
...
% 동역학 행렬 계산 함수 (장치의 관성 추가)
function [M, C, G] = dynamics_matrices(q, qd, m1, m2, L1, L2, I1, I2, m_device, I_device_cm, L_total)
    theta1 = q(1);
    theta2 = q(2);
    theta1_dot = qd(1);
    theta2_dot = qd(2);

    % 병렬 축 정리를 적용하여 장치의 관성 모멘트 계산
    I_device_joint = I_device_cm + m_device * (L_total)^2;

    % 링크 2에 장치의 질량과 관성 추가
    m2_total = m2 + m_device; % 링크 2의 총 질량
    I2_total = I2 + I_device_joint; % 링크 2의 총 관성 모멘트

    % 질량 행렬 M
    M11 = I1 + I2_total + (m1 * (L1^2)) / 4 + m2_total * (L1^2 + (L2^2) / 4 + L1 * L2 * cos(theta2));
    M12 = I2_total + m2_total * ((L2^2) / 4 + (L1 * L2 * cos(theta2)) / 2);
    M21 = M12;
    M22 = I2_total + m2_total * (L2^2) / 4;
    M = [M11, M12; M21, M22];

    % 코리올리 및 원심력 행렬 C
    C11 = -m2_total * L1 * L2 * sin(theta2) * theta2_dot / 2;
    C12 = -m2_total * L1 * L2 * sin(theta2) * (theta1_dot + theta2_dot) / 2;
    C21 = m2_total * L1 * L2 * sin(theta2) * theta1_dot / 2;
    C22 = 0;
    C = [C11, C12; C21, C22];
...
% S자 통로 그리기 함수
function plot_maze(maze_x_center, maze_y_center, maze_width)
    % S자 통로를 그리기 위해 S자 곡선과 폭을 이용하여 상하 경계선을 그립니다.
    num_points = length(maze_x_center);
    for i = 1:num_points-1
        % 각 구간의 선분 그리기 (위쪽 경계)
        x_coords = [maze_x_center(i), maze_x_center(i+1)];
        y_coords_upper = [maze_y_center(i), maze_y_center(i+1)] + maze_width/2;
        plot(x_coords, y_coords_upper, 'k-', 'LineWidth', 2); hold on;
        % 각 구간의 선분 그리기 (아래쪽 경계)
        y_coords_lower = [maze_y_center(i), maze_y_center(i+1)] - maze_width/2;
        plot(x_coords, y_coords_lower, 'k-', 'LineWidth', 2);
    end
end

% 점이 S자 통로 내부에 있는지 확인하는 함수
function [inside_maze, p_corrected] = check_maze(p, maze_x_center, maze_y_center, maze_width)
    x = p(1);
    y = p(2);
    % 현재 x 위치에 대한 y_center 계산 (보간법 사용)
    y_center = interp1(maze_x_center, maze_y_center, x, 'linear', 'extrap');
